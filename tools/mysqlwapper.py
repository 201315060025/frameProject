...
class MySQL:
  ...
  def __init__(self, dbconfig):
    ...
    try:
      ...
      #createdbsql = "create database if not exists %s" % dbconfig['db']
      #self._cur.execute(createdbsql)
      #self._conn.select_db(dbconfig['db'])
      for k, v in dbconfig['tables'].iteritems():
          self._cur.execute(v)
      self._cur.close()
  ...
  def fetchAllRows(self):
    ...
    return self._cur.fetchallDict()

  def fetchOneRow(self):
    u'返回一行结果，然后游标指向下一行。到达最后一行以后，返回None'
    return self._cur.fetchoneDict()
