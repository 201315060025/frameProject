...
from model.hp import Hp
from db.mysqlapp import MySQLApp ## 连接通道 涉及连接通道
from dal.dal_base import Dal_Base  ## 数据称的基类


class Dal_Hp(Dal_Base):
    def __init__(self):
        Dal_Base.__init__(self)

    def addHp(self, newHp):#newAct可以是一个对象   一个对象包含所有的需要传输的信息
       return self.add(newHp) ## add继承基类的属性方法add(),

    def getHp(self, pk):
        return self.get(pk, Hp)

    def updateHp(self, pk, **kwargs):
        return self.update(pk, Hp, **kwargs)

    def deleteHp(self, pk):
