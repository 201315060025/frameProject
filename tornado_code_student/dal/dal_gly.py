...
class Dal_Gly(Dal_Base):
    def __init__(self):
        Dal_Base.__init__(self)

    def addGly(self, newGly):#newAct可以是一个对象   一个对象包含所有的需要传输的信息
       return self.add(newGly) ## add继承基类的属性方法add(),

    def getGly(self, pk):
        return self.get(pk, Gly)

    def updateGly(self, pk, **kwargs):
        return self.update(pk, Gly, **kwargs)
