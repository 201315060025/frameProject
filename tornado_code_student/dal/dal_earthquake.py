...
class Dal_Earthquake(Dal_Base):
    def __init__(self):
        Dal_Base.__init__(self)

    def addEarthquake(self, newEarthquake):#newAct可以是一个对象   一个对象包含所有的需要传输的信息
       return self.add(newEarthquake) ## add继承基类的属性方法add(),

    def getEarthquake(self, pk):
        return self.get(pk, Earthquake)

    def updateEarthquake(self, pk, **kwargs):
        return self.update(pk, Earthquake, **kwargs)

    def deleteEarthquake(self,pk):
        # return self.delete(pk,Act)
        self.delete(pk, Earthquake)
...
#根据某种属性返回满足条件的记录
    ...
    def initCache(self):
        return self.initDB('earthquake', Earthquake)
