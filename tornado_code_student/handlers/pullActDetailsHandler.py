...
class PullActDetailsHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        # temp=Dal_User().getUserByAttr('userid','5')
        # print temp
        ...
        if user == None:
           ...
           idlist = Utils().decodeIDFormat(post_data['actidlist'])
           result = {}

           for k in idlist:
               act = Dal_Act().getAct(k)
               result[k] = {}
               for k1, v1 in act.iteritems():
                    if k1 == 'begintime' or k1 == 'endtime':
        ...
        #处理文件
               ...
               file_path = file_path +'\actPhotos' + '\\' + str(k)
               all_the_text = ''
           ...
           respon = {'errorcode':configs_error['success'], 'actlist':result}
