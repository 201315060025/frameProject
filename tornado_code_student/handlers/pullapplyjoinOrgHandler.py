...
class PullApplyJoinOrgHanler(tornado.web.RequestHandler):
    def post(self):
      post_data={}
      ...
      if post_data['roletype'] != "0":
        ...
        if user == None:
            ...
            for k1 in idlist:
                result[k1]={}
                applyOrgOb=Dal_OrgApply().getOrgApply(k1)
                for k2,v2 in applyOrgOb.iteritems():
                    result[k1][k2]=v2

            ## 以键值对的形式返回 例如：{“1”：{“appler”：“2013”，“reason”:"aaaa"}...}
            response={"errorcode":configs_error['success'],"applyjoinOrg":result}
      print response
