...
class UpdateOrgHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        if user == None:
           respon = {'errorcode':configs_error['userinvaild']}
            ...
            ##检查是不是社长
            bossID = Dal_Org().getOrgTitleMember(post_data['id'],configs_org['orgtitle']['boss'])
            if bossID != user.id:
               ...
               #      upload_path = currpath  + '\orgPhotos' + '\\' + post_data['id']
               ...
               Dal_Org().updateOrg(post_data['id'],**post_data)
               respon = {'errorcode':configs_error['success'], 'orgid':post_data['id']}
